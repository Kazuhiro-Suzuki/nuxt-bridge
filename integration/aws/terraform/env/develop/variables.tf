...
variable "public_subnets" {
  type = map(string)
  default = {
    ap-northeast-1a = "10.1.21.0/24"
    ap-northeast-1c = "10.1.23.0/24"
  }
}
...
variable "monitoring_role_name" {
  type    = string
  default = "LgPwdRdsMonitoringDevRole"
}
...
variable "site_domain" {
  type    = string
  default = "https://dev.lg-pwd.jp"
}
variable "site_sub_domain" {
  type    = string
  default = "https://backend-dev.lg-pwd.jp"
}
...
variable "htp_html_url" {
  type    = string
  default = "https://temp.dev.lg-pwd.jp/index.html"
}
variable "htp_api_url" {
  type    = string
  default = "https://api.dev.htp.ms.mila-e.jp/html-to-pdf"
}
...
variable "backend_domain" {
  type    = string
  default = "backend-dev.lg-pwd.jp"
}
variable "temp_url_domain" {
  type    = string
  default = "temp.dev.lg-pwd.jp"
}
variable "log_bucket" {
  type    = string
...
variable "ecs_task_execution_role_name" {
  type    = string
  default = "LgPwdEcsTaskExecutionDevRole"
}
// ロールを予め作っておく必要がある
// ECS コンテナ内部のロール（アプリケーションで使う権限が必要）
variable "ecs_task_role_name" {
  type    = string
  default = "LgPwdEcsTaskDevRole"
}
// backend のコンテナイメージタグ
variable "image_tag" {
  type    = string
...
variable "aws_ses_configuration_set_name" {
  type    = string
  default = "lg-pwd-develop-configuration-set"
}
variable "tmp_dir" {
  type    = string
  default = "/tmp/lg-pwd"
...
variable "backend_url" {
  type    = string
  default = "https://api-dev.lg-pwd.jp"
}
