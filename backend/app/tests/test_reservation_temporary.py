...
from app.views.reservation_temporary_view import ReservationTemporaryView
...
class ReservationTemporaryPostViewTest(TestCase):
    def setUp(self):
        self.user, self.token = set_up_test_data()
        self.client = APIRequestFactory()
        self.view = ReservationTemporaryView.as_view()
        self.url = '/api/v1/app/reservation_temporary/'
        self.patcher = patch('app.repositories.reservation_temporary_repository.requests.post')
    ...
    def test_200_post_ok(self):
        self.mock_post.side_effect = [MockResponse("""{"id": "3fa85f64-5717-4562-b3fc-2c963f66afa6"}""", 200)]
    ...
    def test_500_post_request_error(self):
        self.mock_post.side_effect = [MockResponse("""{"id": 3fa85f64-5717-4562-b3fc-2c963f66afa6}""", 500)]
        request = self.client.post(self.url, {
            "email": "test-user@gmail.com",
            "slotId": '1',
        })
...
class ReservationTemporaryPutViewTest(TestCase):
    def setUp(self):
        self.user, self.token = set_up_test_data()
        self.client = APIRequestFactory()
        self.view = ReservationTemporaryView.as_view()
...
class ReservationTemporaryGetViewTest(TestCase):
    def setUp(self):
        ...
        self.view = ReservationTemporaryView.as_view()
        self.url = '/api/v1/app/reservation_temporary/'
        self.patcher = patch('app.repositories.reservation_repository.requests.get')
    ...
    def test_500_get_no_email(self):
        self.mock_get.side_effect = [MockResponse("""{"id": "3fa85f64-5717-4562-b3fc-2c963f66afa6"}""", 200)]
        request = self.client.get(self.url, {
            "email": "",
        })
