<template>
    ...
    >
            ...
            <v-col cols="12" class="py-0">
              <v-list-item-avatar :color="darkColor" class="d-flex">
                <v-icon v-if="$auth.user.type === 'general'">
                  $IconThumbnailUserDarkGreen
                </v-icon>
                <v-icon v-else>
                  $IconThumbnailAdminDarkGreen
                </v-icon>
              </v-list-item-avatar>
            </v-col>
            <v-col cols="12" class="py-0">
      ...
      <v-list flat>
          <v-list-item
            class="px-0"
            v-for="(item, i) in userItems.slice(0, 4)"
            :key="i"
            :to="item.to"
            router
            exact
          ...
          <v-list-item
            class="px-0"
            href="https://www.city.hida.gifu.jp/site/sougouhukusi/44031.html"
            target="_blank"
            exact
            link
          >
            <v-list-item-icon>
                <v-icon :color="baseColor">$IconServiceLine</v-icon>
            </v-list-item-icon>
            <v-list-item-content style="display: flex;">
              <div style="display: flex; align-items: center;">
                <div class="pr-3">障がい福祉情報</div>
                <v-icon size="16px" :color="baseColor">$IconNewwindowLine</v-icon>
              </div>
            </v-list-item-content>
          </v-list-item>
          ...
          <v-list-item
            class="px-0"
            v-for="(item, i) in userItems.slice(4, 6)"
            :key="i + 100"
            :to="item.to"
            router
            exact
